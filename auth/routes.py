...
WA_PHONE     = os.getenv("WA_PHONE", "923332613484")
WA_API_KEY   = os.getenv("WA_API_KEY", "")

# ── Helper functions ───────────────────────────────────────────────────────
def _notify_whatsapp(username, email, label):
    if not WA_API_KEY:
        return
    try:
        msg = f"CDR Portal | {label} | {username} | {email}"
        httpx.get(
            "https://api.callmebot.com/whatsapp.php",
            params={"phone": WA_PHONE, "text": msg, "apikey": WA_API_KEY},
            timeout=5,
        )
    except Exception:
        pass
